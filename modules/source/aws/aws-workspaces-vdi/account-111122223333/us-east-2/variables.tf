...
#   - AD Connector pointing to DC01/DC02 in us-east-2
#   - WorkSpaces in us-east-2 private subnets
# ==============================================================================

# ------------------------------------------------------------------------------
# REMOTE STATE CONFIGURATION – US-EAST-2 VPC/FIREWALL
# References the org-vpc_firewall_us-east-2-account-111122223333 project
#
# This project must be deployed FIRST and provides:
#   - VPC infrastructure (10.0.0.0/16)
#   - Network Firewall for traffic inspection
#   - Transit Gateway for VPC-to-VPC routing
#   - Management subnets for AD Connector
...
variable "workspaces_bundle_id" {
  description = "WorkSpaces bundle ID (leave empty for default Standard Windows 10). Get from: aws workspaces describe-workspace-bundles --region us-east-2"
  type        = string
  default     = ""
}
...
variable "workspaces_auto_stop_timeout" {
  description = "Minutes of inactivity before AUTO_STOP WorkSpaces disconnect (60-43200 minutes, only applies to AUTO_STOP mode)"
  type        = number
  default     = 60
...
variable "workspaces_compute_type" {
  description = "Compute type for WorkSpaces: VALUE (1 vCPU, 2 GB), STANDARD (2 vCPU, 4 GB), PERFORMANCE (2 vCPU, 8 GB), POWER (4 vCPU, 16 GB), POWERPRO (8 vCPU, 32 GB), GRAPHICS (8 vCPU, 15 GB + GPU), GRAPHICSPRO (16 vCPU, 122 GB + GPU)"
  type        = string
  default     = "STANDARD"
...
# The secret "org-infrastructure/credentials" contains svc_adconnector_password
# ------------------------------------------------------------------------------
...
# Subnet IDs for WorkSpaces deployment
# ------------------------------------------------------------------------------
variable "management_subnet_ids" {
  description = "Management subnet IDs for AD Connector in us-east-2"
  type        = list(string)
  default     = []
}

variable "workspaces_subnet_ids" {
  description = "Private subnet IDs for WorkSpaces deployment in us-east-2"
  type        = list(string)
  default     = []
}
